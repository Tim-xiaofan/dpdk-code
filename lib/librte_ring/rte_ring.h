...
struct rte_ring_headtail {
	volatile uint32_t head;  /**< Prod/consumer head. 预入/出队的地方*/
	volatile uint32_t tail;  /**< Prod/consumer tail. 实际入/出队的地方*/
 ...
 * field. Thanks to this assumption(设定), we can do subtractions between 2 index
 * values in a modulo-32bit（以32位为模） base: that's why the overflow（溢出） of the indexes is not
 * a problem（即使溢出了，也不会出问题）.
 */
struct rte_ring {
	/*
	...
	uint32_t capacity;       /**< Usable size of ring 容量*/
...
/**
 * @internal Enqueue（入队） several objects on the ring
 *
  * @param r
 *   A pointer to the ring structure.
 * @param obj_table
