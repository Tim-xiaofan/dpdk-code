	       ...
	       uint16_t num_queues)
{
	struct rte_eth_conf port_conf = {
			.rxmode = {
				.mq_mode = ETH_MQ_RX_RSS,//开启网卡rx队列RSS功能
				.split_hdr_size = 0,
			...
			.rx_adv_conf = {
				.rss_conf = {
					.rss_key = NULL,
					.rss_hf = ETH_RSS_TCP,//要求配置的hash function
				},
			},
			.txmode = {
	...
	struct rte_eth_rxconf rxq_conf;//为eth配置rx的结构
	...
	if (rte_eal_process_type() == RTE_PROC_SECONDARY)//secondary procs不操作
		return 0;
	...
	rss_hf_tmp = port_conf.rx_adv_conf.rss_conf.rss_hf;//dpdk要求的
	port_conf.rx_adv_conf.rss_conf.rss_hf &= info.flow_type_rss_offloads;
	if (port_conf.rx_adv_conf.rss_conf.rss_hf != rss_hf_tmp) {//实际网卡配置hash function与dpdk要求配置的RX队列hf不同
		printf("Port %u modified RSS hash function based on hardware support,"
			"requested:%#"PRIx64" configured:%#"PRIx64"\n",
			port,
			rss_hf_tmp,//要求
			port_conf.rx_adv_conf.rss_conf.rss_hf);//实际
	}
...
lcore_main(void *arg __rte_unused)
{
	...
	for (;;) {
		...
		for (p = start_port; p < end_port; p++) {
			...
			const uint16_t rx_c = rte_eth_rx_burst(src, q_id, buf, PKT_BURST);//突发收包，返回实际收包数
			if (rx_c == 0)
				continue;
			pstats[src].rx += rx_c;
			...
			pstats[dst].tx += tx_c;
			if (tx_c != rx_c) {//发送的包与实际收到的包数不相等
				pstats[dst].drop += (rx_c - tx_c);//直接扔掉
				for (i = tx_c; i < rx_c; i++)
...
main(int argc, char **argv)
{
	...
	signal(SIGINT, print_stats);//interrupt
	signal(SIGTERM, print_stats);//Software termination signal from kill
	...
	assign_ports_to_cores();//为每个核分配网卡port
